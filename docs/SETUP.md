# Getting Started with bot_army_inbox

This guide walks you through setting up Inbox Bot for local development.
...
DATABASE_NAME=bot_army_inbox_dev
```
...
Inbox Bot uses the **BotArmy.GenBot** harness to run skills. Skills are discrete, reusable units of work triggered via NATS messages.

### Creating Your First Skill

1. **Start with the template** — `lib/bot_army_inbox/skills/example.ex` is a fully documented template
2. **Copy and rename** it for your skill:
   ```bash
   cp lib/bot_army_inbox/skills/example.ex lib/bot_army_inbox/skills/my_skill.ex
   ```
...
```elixir
defmodule BotArmyInbox.Skills.Classify do
  use BotArmy.Skill

  def name, do: :classify
  def description, do: "Classifies text into categories"
  def nats_triggers, do: ["bot.bot_army_inbox.command.classify"]
...
```elixir
# In lib/bot_army_inbox/application.ex or similar

defmodule BotArmyInbox.MyBot do
  use BotArmy.GenBot,
    skills: [
      BotArmyInbox.Skills.Classify,
      BotArmyInbox.Skills.ExtractEntities
    ],
    bot_id: :my_bot
end
```
...
```bash
# Make your changes
edit lib/bot_army_inbox/skills/my_skill.ex
...
releases: [
  inbox_bot: [
    applications: [bot_army_inbox: :permanent]
  ]
]
```

This creates a release named `inbox_bot` that is deployed to `/opt/ergon/releases/inbox_bot/` on the server.
...
3. Extracts and deploys to `/opt/ergon/releases/inbox_bot/`
