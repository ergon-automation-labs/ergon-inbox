pipeline {
  ...
  environment {
    ...
    GITHUB_REPO = "ergon-automation-labs/ergon-inbox"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/inbox_bot/bin/inbox_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyInbox.Release.create()'"
          $RELEASE_BIN eval 'BotArmyInbox.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyInbox.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyInbox.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_inbox/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_inbox/releases/start_erl.data)
        fi
